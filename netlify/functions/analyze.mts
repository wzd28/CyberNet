...
8. Never invent or claim to have checked a third-party security vendor, antivirus engine, or reputation service. CyberNet AI's analysis is based on its own deterministic rules and your own reasoning only — do not reference VirusTotal, Google Safe Browsing, or any other named external service as having been consulted.
...
20. Write every user-facing field — summary, evidence, and actions — in plain, everyday language a curious 10-year-old could follow. Avoid technical jargon (e.g. instead of "typosquatting" say "a web address that's spelled almost like a real company's, hoping you won't notice"; instead of "credential-harvesting" say "trying to steal your password"). The summary field must be genuinely detailed — at least 3-4 full sentences explaining what the content actually is, why it does or doesn't look suspicious, and what that means for the person reading it in practice. This matters most for plain text messages, where there is no external data to lean on — the explanation itself carries the analysis, so make it thorough, specific to what was actually submitted, and easy to follow, never a generic template.
...
function analyzeTextServer(text: string, label = "Submitted text"): LocalEvidence {
  ...
  signal(/\b(?:legal action|arrest warrant|failure to (?:respond|comply|appear)|face (?:legal|criminal) (?:action|charges)|warrant for your arrest)\b/, 22, "The message threatens legal action, arrest, or criminal charges.", "legal_threat", "Legal or arrest threat");
...
function analyzeEmailHeadersServer(raw: string): LocalEvidence {
  ...
  if (/\bspf=(?:fail|softfail|temperror|permerror|neutral)\b/.test(auth)) addSignal(state, 24, "SPF check failed (a technical check that verifies the sending server was allowed to send mail for this domain).", "spf_failure", { type: "email_auth", value: auth.match(/spf=[a-z]+/)?.[0] || "spf failure", riskReason: "The sending server was not clearly authorized by SPF." });
  if (/\bdkim=(?:fail|temperror|permerror|neutral)\b/.test(auth)) addSignal(state, 24, "DKIM check failed (a technical check that verifies the email wasn't altered in transit).", "dkim_failure", { type: "email_auth", value: auth.match(/dkim=[a-z]+/)?.[0] || "dkim failure", riskReason: "The cryptographic message signature was not validated." });
  if (/\bdmarc=(?:fail|temperror|permerror|neutral)\b/.test(auth)) addSignal(state, 30, "DMARC check failed (a technical check that ties the visible sender name to the actual sending domain).", "dmarc_failure", { type: "email_auth", value: auth.match(/dmarc=[a-z]+/)?.[0] || "dmarc failure", riskReason: "The visible From domain did not satisfy DMARC alignment." });
...
// NOTE (2026-08-26): This file previously called Google Safe Browsing and
// VirusTotal's free/public APIs directly. Both explicitly prohibit commercial
// use in their own terms of service — Google's docs direct commercial users
// to the separate paid Web Risk API, and VirusTotal's public API documentation
// states it "must not be used in commercial products or services." Since
// CyberNet AI is a paid product, those integrations were removed entirely
// (not just gated behind a missing key) to avoid reintroducing a real ToS
// violation. If real external threat-intel is added in the future, it must
// go through a provider's actual commercial/paid tier with a signed agreement,
// not a free/community tier.
...
export default async function handler(request: Request, context: any): Promise<Response> {
  ...
  // NOTE: Google Safe Browsing and VirusTotal's free/public APIs both explicitly
  // prohibit commercial use (confirmed directly from their own terms of service).
  // Since CyberNet AI is a paid commercial product, calling either API here would
  // be a real ToS violation. These checks are intentionally disabled rather than
